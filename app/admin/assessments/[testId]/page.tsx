...
const [customQuestion, setCustomQuestion] = useState({
  ...
  try {
    ...
    const response = await fetch(
      `/api/admin/assessments/${testId}/questions/${question.id}`,
      {
        method: "DELETE",
        headers: {
          Authorization: `Bearer ${session.access_token}`,
...
async function handleOpenSwapOptions(question: TestQuestion) {
  ...
  try {
    ...
    const response = await fetch(
      `/api/admin/assessments/${testId}/questions/${question.id}/swap-options`,
      {
        headers: {
          Authorization: `Bearer ${session.access_token}`,
        },
...
async function handleAddCustomQuestion() {
  ...
  try {
    ...
    const response = await fetch(
      `/api/admin/assessments/${testId}/questions/custom`,
      {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
...
async function handlePublishTest() {
  ...
  const confirmed = window.confirm(
    "Publish this test now? Students with the share link will be able to start attempting it."
  );
  ...
  try {
    ...
    const response = await fetch(`/api/admin/assessments/${testId}`, {
      method: "PATCH",
      headers: {
        Authorization: `Bearer ${session.access_token}`,
      },
    ...
    setPublishSuccess("Test published successfully.");
  } catch (publishError) {
    setActionError(
...
async function handleCopyShareLink() {
  ...
  const shareLink = `${window.location.origin}/test/${draft.test.share_code}`;

  try {
    await navigator.clipboard.writeText(shareLink);
    setPublishSuccess("Student test link copied.");
  } catch {
    setActionError(
      "Could not copy the link automatically. Please copy it from the browser address bar once the student page is added."
    );
  }
}
  if (loading) {
...
<section className="mt-6 rounded-3xl border border-slate-200 bg-white p-5 shadow-sm sm:p-6">
  ...
  {test.status === "draft" ? (
    ...
    <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
      <div>
        ...
        <code className="mt-2 block break-all rounded-xl bg-slate-100 px-3 py-2.5 text-xs font-medium text-slate-800">
          {typeof window !== "undefined"
            ? `${window.location.origin}/test/${test.share_code}`
            : `/test/${test.share_code}`}
        </code>
      </div>

     <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
  <Link
    href={`/admin/assessments/${test.id}/results`}
  ...
  <button
    ...
    className="inline-flex min-h-11 items-center justify-center gap-2 rounded-xl border border-slate-200 bg-white px-5 py-3 text-sm font-semibold text-slate-900 transition hover:bg-slate-50"
  >
